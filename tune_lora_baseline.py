...
class Trainer(object):
	...
	def __init__(self, args, logger):
		...
		# setup tokenizer
		logger.info(f"Loading Model tokenizer from {get_huggingface_path(args.model_type)}")
		...
		with self.accelerator.main_process_first():
			...
			model_trainable_params, model_all_params = self.__count_parameters(self.model)
			msg = (f"Model: trainable params: {model_trainable_params:,d} || all params: {model_all_params:,d} ||"
				   f" trainable%: {100 * model_trainable_params / model_all_params}")
	...
	def __count_parameters(model):
		dec_trainable_params, dec_all_params = model.get_nb_trainable_parameters()
				...
				self.accelerator.log(
					{
						...
						"Step/Model Learning Rate": self.optimizer.param_groups[0]["lr"],
					},
					step=self.step,
				)
