...
class PeftCvaeModelForCausalLM(PeftCvaeModel):
	...
	def forward(
		...
		if peft_config.peft_type == PeftType.PREFIX_TUNING:
			...
			prompts = prompts * latent_attention_weights.unsqueeze(-1)
	...
	def prepare_inputs_for_generation(self, *args, **kwargs):
		...
		if peft_config.is_prompt_learning:
			...
			latent_attention_weights = self.latent_attention_weights
			# if latent_attention_weights is None:
			# 	raise ValueError("latent_attention_weights is not set before generate")
			...
			if model_kwargs["past_key_values"] is None and peft_config.peft_type == PeftType.PREFIX_TUNING:
				...
				# Added functionality
				if latent_attention_weights is not None:
					past_key_values = past_key_values * latent_attention_weights.unsqueeze(-1)
