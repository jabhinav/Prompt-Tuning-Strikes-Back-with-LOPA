...
class ClarificationCodeBERTPredictor(torch.nn.Module):
	def __init__(self, args, output_dim):
		...
		# Mean head - Mu
		self.dropout_mu = torch.nn.Dropout(config.hidden_dropout_prob)
		self.dense_mu = torch.nn.Linear(config.hidden_size, config.hidden_size)
		self.out_proj_mu = torch.nn.Linear(config.hidden_size, output_dim)
		
		# Log-variance head - LogVar
		self.dropout_logvar = torch.nn.Dropout(config.hidden_dropout_prob)
		self.dense_logvar = torch.nn.Linear(config.hidden_size, config.hidden_size)
		self.out_proj_logvar = torch.nn.Linear(config.hidden_size, output_dim)
		
		self.init_predictor_head()
	
	def init_predictor_head(self):
		# Initialize the weights
		self.dense_mu.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
		self.out_proj_mu.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
		self.out_proj_mu.bias.data.zero_()
		
		self.dense_logvar.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
		self.out_proj_logvar.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
		self.out_proj_logvar.bias.data.zero_()
	...
	def forward(self, input_ids, attention_mask):
		...
		# Predict the mean
		mu = self.dropout_mu(x)
		mu = self.dense_mu(mu)
		mu = torch.nn.functional.tanh(mu)
		mu = self.dropout_mu(mu)
		mu = self.out_proj_mu(mu)
		
		# Predict the log-variance
		logvar = self.dropout_logvar(x)
		logvar = self.dense_logvar(logvar)
		logvar = torch.nn.functional.tanh(logvar)
		logvar = self.dropout_logvar(logvar)
		logvar = self.out_proj_logvar(logvar)
		return mu, logvar
...
class Trainer(object):
	...
	def init_decoder(self):
		...
		# Load checkpoint
		if self.args.load_base_from_path is not None:
			# We load the model state dict on the CPU to avoid an OOM error.
			loaded_state_dict = torch.load(self.args.load_base_from_path, map_location="cpu")
			...
			msg = "Loaded decoder base checkpoint from path: {}".format(self.args.load_base_from_path)
			if is_rank_0():
				print(msg)
		...
		if self.args.load_adapter_from is not None:
			# Load the model adapters - in place
			model = PeftMultiModel.from_pretrained(
				model=model,
				model_id=self.args.load_adapter_from,  # Must be a directory containing the model files
				config=peft_config,
			)
			msg = "[INFO] Loaded the model adapters from: {}".format(self.args.load_adapter_from)
			if is_rank_0():
				print(msg)
		else:
	...
	def init_encoder(self):
		...
		model = ClarificationCodeBERTPredictor(
			args=self.args,
			output_dim=self.args.total_virtual_tokens * self.args.word_embedding_dim
		)
		
		if self.args.clf_predictor_path is not None:
			# Load the model state dict on the CPU to avoid an OOM error.
			loaded_state_dict = torch.load(self.args.clf_predictor_path, map_location="cpu")
			loaded_state_dict = {k.replace('module.', ''): v for k, v in loaded_state_dict.items()}
			model.load_state_dict(loaded_state_dict, strict=True)
			...
			# Log the loaded checkpoint
			msg = "Loaded encoder checkpoint from path: {}".format(self.args.clf_predictor_path)
			if is_rank_0():
				print(msg)
	...
	def get_response_log_probs(self, batch, tokenizer, model, latent_prompts):
	...
	def _train_step(self, batch):
		...
		"""
		# Encode
		clf_mean, log_var = self.model["enc"](*batch[:2])
		# For constant variance
		# log_var = torch.zeros_like(clf_mean)
		
		# KL Divergence: D(q(z|x) || p(z)) = - 0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
		# Help: https://stats.stackexchange.com/questions/318748/deriving-the-kl-divergence-loss-for-vaes
		kl_div = torch.mean(-0.5 * torch.sum(1 + log_var - clf_mean ** 2 - log_var.exp(), dim=1), dim=0)
		
		# Sample using re-parameterization trick
		latent_prompts = self.reparameterize(clf_mean, log_var)
		# Reshape the latent prompts [B, n*k] -> [B, n, k], n:=total_virtual_tokens, k:=hidden_size
		latent_prompts = latent_prompts.view(-1, self.args.total_virtual_tokens, self.args.word_embedding_dim)
